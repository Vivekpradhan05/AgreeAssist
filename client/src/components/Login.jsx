...
// Allow user login or register on there website
  ...
  return (
    // in this file chenge the font text color to primary it cant be set In video time 2:20:00
    ...
    <form onSubmit={onSubmitHandler} onClick={(e)=>e.stopPropagation()} className="flex flex-col gap-4 m-auto items-start p-8 py-12 w-80 sm:w-[352px] text-gray-500 rounded-lg shadow-xl border border-gray-200 bg-white">
            <p className="text-2xl font-medium m-auto">
                <span className="text-sky">User</span> {state === "login" ? "Login" : "Sign Up"}
            </p>
            {state === "register" && (
            ...
            {state === "register" ? (
                <p>
                    Already have account? <span onClick={() => setState("login")} className="text-sky cursor-pointer">click here</span>
                </p>
            ) : (
                <p>
                    Create an account? <span onClick={() => setState("register")} className="text-sky cursor-pointer ">click here</span>
                </p>
            )}
            <button className="bg-sky-500 hover:bg-sky-700 transition-all text-white w-full py-2 rounded-md cursor-pointer">
